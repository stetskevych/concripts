...
for instance_name in "${INSTANCES[@]}"; do
  ...
  log "-----"
  log "Backing up $instance_name"
  if ! instance_id="$(aws ec2 describe-instances --output json --filters "Name=tag:Name,Values=$instance_name" | grep "InstanceId" | cut -d\" -f4)"; then
    exit_fail "Error getting instance ID"
  fi

  log "Creating AMI of $instance_id"
  if ! aws ec2 create-image --output json --instance-id "$instance_id" --name "${instance_name}-ami-${DATE}" --no-reboot --description "Automatic backup by $SCRIPTNAME on $HOSTNAME"; then
    exit_fail "Error creating AMI for $instance_id"
  fi

  log "Cleaning up older AMIs (AMI_RETENTION = $AMI_RETENTION)"
  if ! ami_list="$(aws ec2 describe-images --output json --owners self --filters "Name=name,Values=${instance_name}-ami-*" | grep '\"Name\":' | cut -d\" -f4 | sort -r)"; then
  ...
  if [ ! -z "$ami_list_to_delete" ]; then
    while read -r ami_to_delete; do
      if ! snapshot_list_to_delete="$(aws ec2 describe-images --output json --owners self --filters "Name=name,Values=$ami_to_delete" | grep '\"SnapshotId\":' | cut -d\" -f4)"; then
        exit_fail "Error getting a list of snapshot IDs for $ami_to_delete"
      fi

      if ! ami_id_to_delete="$(aws ec2 describe-images --output json --owners self --filters "Name=name,Values=$ami_to_delete" | grep '\"ImageId\":' | cut -d\" -f4)"; then
        exit_fail "Error getting AMI ID for $ami_to_delete"
      fi

      log "Deregistering $ami_to_delete ($ami_id_to_delete)"
      if ! aws ec2 deregister-image --output json --image-id "$ami_id_to_delete"; then
      ...
      while read -r snapshot_to_delete; do
        ...
        if ! aws ec2 delete-snapshot --output json --snapshot-id "$snapshot_to_delete"; then
